...
std::string handleRadars(const http::request<http::string_body>& req) {
    ...
    if (req.method() == http::verb::get) {
        ...
        resp = g_scene_mgr.addRadar(sid, rad)
                   ? jsonOk()
                   : jsonErr("add failed");
    } else {
...
std::string handleRadar(const http::request<http::string_body>& req) {
    ...
    if (req.method() == http::verb::post) {
        ...
        if (action == "del") {
            ...
            resp = g_scene_mgr.removeRadar(sid, rid)
                       ? jsonOk()
                       : jsonErr("not found");
            ...
            ECMSim::Radar rad(
                ...
                jr["sigma"].asDouble(),
                jr["thresh_db"].asDouble());
            resp = g_scene_mgr.updateRadar(sid, rid, rad)
                       ? jsonOk()
                       : jsonErr("update failed");
        } else {
            resp = jsonErr("unknown action");
        }
...
std::string handleJammers(const http::request<http::string_body>& req) {
    ...
    if (req.method() == http::verb::get) {
        ...
        ECMSim::Jammer jam(
            ...
            jt);
        resp = g_scene_mgr.addJammer(sid, jam)
                   ? jsonOk()
                   : jsonErr("add failed");
    } else {
        resp = jsonErr("use GET/POST");
...
std::string handleJammer(const http::request<http::string_body>& req) {
    ...
    if (req.method() == http::verb::post) {
        ...
        if (action == "del") {
            ...
            resp = g_scene_mgr.removeJammer(sid, jid)
                       ? jsonOk()
                       : jsonErr("not found");
        } else if (action == "update") {
            auto jv = body["jammer"];
            ...
            ECMSim::Jammer jam(
                ...
                jv["jam_freq"].asDouble(),
                jt);
            resp = g_scene_mgr.updateJammer(sid, jid, jam)
                       ? jsonOk()
                       : jsonErr("update failed");
        } else {
            resp = jsonErr("unknown action");
...
private:
    ...
    void process() {
        ...
        bool is_html = (svToString(req_.target()) == "/");
